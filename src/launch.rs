...
/// Byte capacity of a built search URL.
...
fn build_search_url(template: &str, query: &str, out: &mut ArrayString<URL_CAP>) {
    let mut encoded: ArrayString<URL_CAP> = ArrayString::new();
    percent_encode(query, &mut encoded);
    if let Some(pos) = template.find("%s") {
        let _ = out.push_str(&template[..pos]);
        let _ = out.push_str(encoded.as_str());
        let _ = out.push_str(&template[pos + 2..]);
    } else {
        let _ = out.push_str(template);
        let _ = out.push_str(encoded.as_str());
    }
}
...
fn percent_encode<const N: usize>(s: &str, out: &mut ArrayString<N>) {
    for byte in s.bytes() {
        match byte {
